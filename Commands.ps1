...
function Show-Posh-Sdk-Version() {
    ...
    Write-Output "posh-sdk (POwer SHell Groovy enVironment Manager) $poshSdkVersion base on SDK $SDK_BASE_VERSION and SDK API $apiVersion"
}
...
function Set-Offline-Mode($Flag) {
    ...
    switch ($Flag) {
        ...
        'disable' { $Script:SDK_FORCE_OFFLINE = $false; $Script:SDK_ONLINE = $true; Write-Output 'Online mode re-enabled!' }
    ...
    switch ($DataType) {
        ...
        'version'    {
                        if ( Test-Path $Script:SDK_API_VERSION_PATH ) {
                            Remove-Item $Script:SDK_API_VERSION_PATH
